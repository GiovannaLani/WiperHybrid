Shader "Unlit/Window"
{
    Properties
    {
        _MainTex("Texture", 2D) = "white" {}   // Textura del cristal
        _Size("Size", float) = 1     // Máscara del limpiaparabrisas
        _T("Time", float) = 1 // Normal Map de agua
        _Distortion("Distortion", range(-5,5)) = 1 // Control de transparencia
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 Layer(float2 UV, float t) {
                ...
                float fogTrail = S(-.05, .05, dropPos.y); // Corregido a trailFactor
                fogTrail *= S(.5, y, gv.y);
                trail *= fogTrail;
